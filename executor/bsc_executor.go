...
import (
	...
	"github.com/MocaFoundation/moca-relayer/contract/crosschain"
	"github.com/MocaFoundation/moca-relayer/contract/mocalightclient"
	"github.com/MocaFoundation/moca-relayer/contract/relayerhub"
	"github.com/MocaFoundation/moca-relayer/logging"
	"github.com/MocaFoundation/moca-relayer/metric"
	rtypes "github.com/MocaFoundation/moca-relayer/types"
)
...
type BSCClient struct {
	...
	mocaLightClient  *mocalightclient.MocaLightClient
...
func newBSCClients(config *config.Config) []*BSCClient {
	...
	for _, provider := range config.BSCConfig.RPCAddrs {
		...
		mocaLightClient, err := mocalightclient.NewMocaLightClient(
			common.HexToAddress(config.RelayConfig.MocaLightClientContractAddr),
			ethClient)
		if err != nil {
			panic("new moca light client error")
		}
		...
		bscClients = append(bscClients, &BSCClient{
			...
			mocaLightClient:  mocaLightClient,
...
func (e *BSCExecutor) SetMocaExecutor(ge *MocaExecutor) {
	e.MocaExecutor = ge
}
...
func (e *BSCExecutor) GetMocaLightClient() *mocalightclient.MocaLightClient {
	e.mutex.RLock()
	defer e.mutex.RUnlock()
	return e.bscClients[e.clientIdx].mocaLightClient
}
...
// GetNextDeliveryOracleSequenceWithRetry gets the next delivery Oracle sequence from Moca
...
func (e *BSCExecutor) getNextDeliveryOracleSequence(chainId sdk.ChainID) (uint64, error) {
	sequence, err := e.MocaExecutor.GetNextReceiveOracleSequence(chainId)
	if err != nil {
		return 0, err
	}
	return sequence, nil
...
func (e *BSCExecutor) SyncTendermintLightBlock(height uint64) (common.Hash, error) {
	...
	oldcsbts, err := e.GetMocaLightClient().ConsensusStateBytes(nil)
	if err != nil {
		return common.Hash{}, err
	}
	// logging.Logger.Debugf("moca-contracts ConsensusStateBytes: %s", hex.EncodeToString(oldcsbts))
	...
	tx, err := e.GetMocaLightClient().SyncLightBlock(txOpts, result, height)
	if err != nil {
		return common.Hash{}, err
	}
...
func (e *BSCExecutor) QueryTendermintLightBlockWithRetry(height int64) (lightBlock tmtypes.LightBlock, err error) {
	return lightBlock, retry.Do(func() error {
		lightBlock, err = e.MocaExecutor.QueryTendermintLightBlock(height)
		return err
	}, relayercommon.RtyAttem,
		relayercommon.RtyDelay,
		relayercommon.RtyErr,
...
func (e *BSCExecutor) QueryLatestTendermintHeaderWithRetry() (lightBlockBts []byte, err error) {
	latestHeigh, err := e.MocaExecutor.GetLatestBlockHeight()
	if err != nil {
		return nil, err
	}
	return lightBlockBts, retry.Do(func() error {
		lightBlock, err := e.MocaExecutor.QueryTendermintLightBlock(int64(latestHeigh))
...
func (e *BSCExecutor) QueryLatestValidators() ([]rtypes.Validator, error) {
	relayerAddresses, err := e.GetMocaLightClient().GetRelayers(nil)
	if err != nil {
		return nil, err
	}
	blsKeys, err := e.GetMocaLightClient().BlsPubKeys(nil)
	if err != nil {
		return nil, err
	}
...
func (e *BSCExecutor) GetLightClientLatestHeight() (uint64, error) {
	...
	latestHeight, err := e.GetMocaLightClient().MocaHeight(callOpts)
	if err != nil {
		return 0, err
	}
...
func (e *BSCExecutor) GetInturnRelayer() (*rtypes.InturnRelayer, error) {
	...
	r, err := e.GetMocaLightClient().GetInturnRelayer(callOpts)
	if err != nil {
		return nil, err
	}
...
// ClaimRewardLoop relayer would claim the reward if its balance is below 1moca and the Reward is over 0.1moca.
// if after refilled with the rewards, its balance is still lower than 1moca, it will keep alerting
func (e *BSCExecutor) ClaimRewardLoop() {
	...
	for range ticker.C {
		...
		balance.Div(balance, MOCADecimal)

		e.metricService.SetBSCBalance(float64(balance.Int64()))

		// should not claim if balance > 1 moca
