...
func NewMocaAssembler(cfg *config.Config, executor *executor.MocaExecutor, dao *dao.DaoManager, bscExecutor *executor.BSCExecutor,
	ms *metric.MetricService,
) *MocaAssembler {
	channels := cfg.MocaConfig.MonitorChannelList
	...
	return &MocaAssembler{
		config:                         cfg,
		mocaExecutor:                   executor,
		daoManager:                     dao,
...
func (a *MocaAssembler) AssembleTransactionsLoop() {
...
func (a *MocaAssembler) assembleTransactionAndSendForChannel(channelId types.ChannelId, inturnRelayer *types.InturnRelayer, isInturnRelyer bool, wg *sync.WaitGroup) {
	defer wg.Done()
...
func (a *MocaAssembler) process(channelId types.ChannelId, inturnRelayer *types.InturnRelayer, isInturnRelyer bool) error {
	var (
		startSeq    uint64
		endSequence int64
	)
	...
	if isInturnRelyer {
		if !a.inturnRelayerSequenceStatusMap[channelId].HasRetrieved {
			...
			inTurnRelayerStartSeq, err := a.mocaExecutor.GetNextDeliverySequenceForChannelWithRetry(channelId)
			if err != nil {
		...
		var err error
		startSeq, err = a.mocaExecutor.GetNextDeliverySequenceForChannelWithRetry(channelId)
		if err != nil {
			return fmt.Errorf("faield to get next delivery sequence for channel %d, err=%s", channelId, err.Error())
		}
	}
	...
	if isInturnRelyer {
		endSequence, err = a.daoManager.MocaDao.GetLatestSequenceByChannelIdAndStatus(channelId, db.AllVoted)
		if err != nil {
		...
		endSeq, err := a.mocaExecutor.GetNextSendSequenceForChannelWithRetry(a.getDestChainId(), channelId)
		if err != nil {
	...
	for i := startSeq; i <= uint64(endSequence); i++ {
		...
		if !isInturnRelyer && time.Now().Unix() < tx.TxTime+a.config.RelayConfig.MocaToBSCInturnRelayerTimeout {
			return nil
		}
...
func (a *MocaAssembler) processTx(tx *model.MocaRelayTransaction, nonce uint64, isInturnRelyer bool) error {
	...
	relayerAddresses, errs := a.bscExecutor.GetMocaLightClient().GetRelayers(nil)
	if errs != nil {
		return fmt.Errorf("GetMocaLightClient GetRelayers failed, errs=%s", errs.Error())
	}
	// blsKeys, errs := a.bscExecutor.GetMocaLightClient().BlsPubKeys(nil)
	// if err != nil {
	// 	return fmt.Errorf("GetMocaLightClient BlsPubKeys failed, errs=%s", errs.Error())
	// }
	...
	if !isInturnRelyer {
		if err = a.daoManager.MocaDao.UpdateTransactionClaimedTxHash(tx.Id, txHash.String()); err != nil {
			return fmt.Errorf("failed to update transaciton status, err=%s", err.Error())
		}
	}

	if err = a.daoManager.MocaDao.UpdateTransactionStatusAndClaimedTxHash(tx.Id, db.Delivered, txHash.String()); err != nil {
		return fmt.Errorf("failed to update transaciton status, err=%s", err.Error())
	}
	a.mutex.Lock()
...
func (a *MocaAssembler) getMonitorChannels() []uint8 {
	return a.config.MocaConfig.MonitorChannelList
}

func (a *MocaAssembler) updateMetrics(channelId types.ChannelId, nextDeliverySeq uint64) error {
	a.metricService.SetNextReceiveSequenceForChannel(uint8(channelId), nextDeliverySeq)
	nextSendSeq, err := a.mocaExecutor.GetNextSendSequenceForChannelWithRetry(a.getDestChainId(), channelId)
	if err != nil {
		return err
	}
...
func (a *MocaAssembler) getDestChainId() sdk.ChainID {
