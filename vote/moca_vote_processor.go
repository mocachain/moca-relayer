...
import (
	...
	rcommon "github.com/mocachain/moca-relayer/common"
...
func NewMocaVoteProcessor(cfg *config.Config, dao *dao.DaoManager, signer *VoteSigner,
	mocaExecutor *executor.MocaExecutor,
) *MocaVoteProcessor {
	...
	return &MocaVoteProcessor{
		config:       cfg,
		daoManager:   dao,
		signer:       signer,
		mocaExecutor: mocaExecutor,
		blsPublicKey: mocaExecutor.BlsPubKey,
		eventType:    eventType,
	}
}

// SignAndBroadcastLoop signs tx using the relayer's bls private key, then broadcasts the vote to Moca votepool
func (p *MocaVoteProcessor) SignAndBroadcastLoop() {
...
func (p *MocaVoteProcessor) signAndBroadcast() error {
	txs, err := p.daoManager.MocaDao.GetTransactionsByStatusWithLimit(db.Saved, p.config.VotePoolConfig.VotesBatchMaxSizePerInterval)
	if err != nil {
	...
	for _, tx := range txs {
		...
		if isFilled {
			if err = p.daoManager.MocaDao.UpdateTransactionStatus(tx.Id, db.Delivered); err != nil {
				return err
			}
		...
		if err = retry.Do(func() error {
			...
			err = p.mocaExecutor.BroadcastVote(v)
			if err != nil {
		...
		err = p.daoManager.MocaDao.DB.Transaction(func(dbTx *gorm.DB) error {
...
func (p *MocaVoteProcessor) CollectVotesLoop() {
...
func (p *MocaVoteProcessor) collectVotes() error {
	txs, err := p.daoManager.MocaDao.GetTransactionsByStatusWithLimit(db.SelfVoted, p.config.VotePoolConfig.VotesBatchMaxSizePerInterval)
	if err != nil {
...
func (p *MocaVoteProcessor) collectVoteForTx(tx *model.MocaRelayTransaction, errChan chan error, wg *sync.WaitGroup) {
	defer wg.Done()
	isFilled, err := p.isTxSequenceFilled(tx)
	if err != nil {
		errChan <- err
	...
	if err = p.daoManager.MocaDao.UpdateTransactionStatus(tx.Id, db.AllVoted); err != nil {
		errChan <- err
		return
	}
}
...
func (p *MocaVoteProcessor) prepareEnoughValidVotesForTx(tx *model.MocaRelayTransaction) error {
	...
	validators, err := p.mocaExecutor.BscExecutor.QueryCachedLatestValidators()
	if err != nil {
		return err
	}
...
func (p *MocaVoteProcessor) queryMoreThanTwoThirdVotesForTx(localVote *model.Vote, validators []types.Validator) error {
	triedTimes := 0
	...
	for range ticker.C {
		...
		queriedVotes, err := p.mocaExecutor.QueryVotesByEventHashAndType(localVote.EventHash, p.eventType)
		if err != nil {
...
func (p *MocaVoteProcessor) constructVoteAndSign(aggregatedPayload []byte) *votepool.Vote {
	var v votepool.Vote
...
func (p *MocaVoteProcessor) getEventHash(aggregatedPayload []byte) []byte {
	return crypto.Keccak256Hash(aggregatedPayload).Bytes()
}

func (p *MocaVoteProcessor) isVotePubKeyValid(v *votepool.Vote, validators []types.Validator) bool {
...
func (p *MocaVoteProcessor) aggregatePayloadForTx(tx *model.MocaRelayTransaction) ([]byte, error) {
...
func (p *MocaVoteProcessor) txFeeToBytes(txFee string) ([]byte, error) {
...
func (p *MocaVoteProcessor) isTxSequenceFilled(tx *model.MocaRelayTransaction) (bool, error) {
	nextDeliverySequence, err := p.mocaExecutor.GetNextDeliverySequenceForChannelWithRetry(types.ChannelId(tx.ChannelId))
	if err != nil {
...
func (p *MocaVoteProcessor) reBroadcastVote(localVote *model.Vote) error {
	...
	return p.mocaExecutor.BroadcastVote(v)
}
