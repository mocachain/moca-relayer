...
func NewBSCVoteProcessor(cfg *config.Config, dao *dao.DaoManager, signer *VoteSigner, bscExecutor *executor.BSCExecutor) *BSCVoteProcessor {
	...
	return &BSCVoteProcessor{
		...
		blsPublicKey: bscExecutor.MocaExecutor.BlsPubKey,
		eventType:    eventType,
	}
}
...
func (p *BSCVoteProcessor) signAndBroadcast() error {
	...
	for seq, pkgsForSeq := range pkgsGroupByOracleSeq {
		...
		// check if oracle sequence is filled on moca, if so, update packages status to filled and skip to next oracle sequence
		...
		blsClaim := oracletypes.BlsClaim{
			...
			DestChainId: uint32(p.config.MocaConfig.ChainId),
		...
		if err = retry.Do(func() error {
			err = p.bscExecutor.MocaExecutor.BroadcastVote(v)
			if err != nil {
...
func (p *BSCVoteProcessor) prepareEnoughValidVotesForPackages(channelId types.ChannelId, sequence uint64, pkgIds []int64) error {
	...
	validators, err := p.bscExecutor.MocaExecutor.QueryCachedLatestValidators()
	if err != nil {
		return err
	}
...
func (p *BSCVoteProcessor) queryMoreThanTwoThirdValidVotes(localVote *model.Vote, validators []*tmtypes.Validator) error {
	...
	for range ticker.C {
		...
		queriedVotes, err := p.bscExecutor.MocaExecutor.QueryVotesByEventHashAndType(localVote.EventHash, p.eventType)
		if err != nil {
...
func (p *BSCVoteProcessor) isOracleSequenceFilled(seq uint64) (bool, error) {
	nextDeliverySeqOnMoca, err := p.bscExecutor.GetNextDeliveryOracleSequenceWithRetry(p.getChainId())
	if err != nil {
		return false, fmt.Errorf("failed to get next delivery oracleSequence, err=%s", err.Error())
	}
	return seq < nextDeliverySeqOnMoca, nil
}
...
func (p *BSCVoteProcessor) reBroadcastVote(localVote *model.Vote) error {
	...
	return p.bscExecutor.MocaExecutor.BroadcastVote(v)
}
