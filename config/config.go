...
type Config struct {
	MocaConfig     MocaConfig     `json:"moca_config"`
...
func (cfg *MocaConfig) Validate() {
	if cfg.RPCAddrs == nil || len(cfg.RPCAddrs) == 0 {
		panic("provider address of Moca should not be empty")
	}

	if cfg.EVMAddrs == nil || len(cfg.EVMAddrs) == 0 {
		panic("provider evm address of Moca should not be empty")
	}

	if cfg.KeyType == "" {
		panic("key_type Moca should not be empty")
	}
	if cfg.KeyType != KeyTypeLocalPrivateKey && cfg.KeyType != KeyTypeAWSPrivateKey {
		panic(fmt.Sprintf("key_type of Moca only supports %s and %s", KeyTypeLocalPrivateKey, KeyTypeAWSPrivateKey))
	}
	if cfg.KeyType == KeyTypeAWSPrivateKey && cfg.AWSRegion == "" {
		panic("aws_region of Moca should not be empty")
	}
	if cfg.KeyType == KeyTypeAWSPrivateKey && cfg.AWSSecretName == "" {
		panic("aws_secret_name of Moca should not be empty")
	}
	if cfg.KeyType != KeyTypeAWSPrivateKey && cfg.PrivateKey == "" {
		panic("privateKey of Moca should not be empty")
	}
...
type RelayConfig struct {
	BSCToMocaInturnRelayerTimeout int64  `json:"bsc_to_moca_inturn_relayer_timeout"` // in second
	MocaToBSCInturnRelayerTimeout int64  `json:"moca_to_bsc_inturn_relayer_timeout"` // in second
	MocaSequenceUpdateLatency     int64  `json:"moca_sequence_update_latency"`       // in second
	...
	MocaLightClientContractAddr   string `json:"moca_light_client_contract_addr"`
	RelayerHubContractAddr        string `json:"relayer_hub_contract_addr"`
	SrcMocaSBTContractAddr        string `json:"src_mocasbt_contract_addr"`
	SrcMocaVCContractAddr         string `json:"src_mocavc_contract_addr"`
}

func (cfg *RelayConfig) Validate() {
	if cfg.BSCToMocaInturnRelayerTimeout <= 0 {
		panic("bsc_to_moca_inturn_relayer_timeout should be larger than 0")
	}
	if cfg.MocaToBSCInturnRelayerTimeout <= 0 {
		panic("moca_to_bsc_inturn_relayer_timeout should be larger than 0")
	}
	if cfg.MocaSequenceUpdateLatency <= 0 {
		panic("moca_sequence_update_latency should be larger than 0")
	}
