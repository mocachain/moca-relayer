...
const (
	MetricNameGnfdSavedBlock       = "Moca_saved_block_height"
	MetricNameGnfdProcessedBlock   = "Moca_processed_block_height"
	MetricNameIsGnfdInturnRelayer  = "is_Moca_inturn_relayer"
	MetricNameGnfdRelayerStartTime = "Moca_relayer_start_time" // inturn relayer start time
	MetricNameGnfdRelayerEndTime   = "Moca_relayer_end_time"   // inturn relayer end time
...
func NewMetricService(config *config.Config) *MetricService {
	...
	labels := map[string]string{
		"gnfdChainId": config.MocaConfig.ChainIdString,
	...
	gnfdSavedBlockMetric := prometheus.NewGauge(prometheus.GaugeOpts{
		...
		Help:        "Saved block height for Moca in Database",
		ConstLabels: labels,
	})
	...
	gnfdProcessedBlockMetric := prometheus.NewGauge(prometheus.GaugeOpts{
		...
		Help:        "Processed block height for Moca in Database",
		ConstLabels: labels,
	})
	...
	gnfdIsInturnRelayerMetric := prometheus.NewGauge(prometheus.GaugeOpts{
		...
		Help:        "Whether relayer is inturn to relay transaction from BSC to Moca",
		ConstLabels: labels,
	})
	ms[MetricNameIsGnfdInturnRelayer] = gnfdIsInturnRelayerMetric
	prometheus.MustRegister(gnfdIsInturnRelayerMetric)

	// Moca relayer(BSC -> Moca) relay interval metrics
	...
	bscIsInturnRelayerMetric := prometheus.NewGauge(prometheus.GaugeOpts{
		...
		Help:        "Whether relayer is inturn to relay transaction from Moca to BSC",
		ConstLabels: labels,
	})
	ms[MetricNameIsBSCInturnRelayer] = bscIsInturnRelayerMetric
	prometheus.MustRegister(bscIsInturnRelayerMetric)

	// BSC relayer(Moca -> BSC) relay interval metrics
	...
	for _, c := range config.MocaConfig.MonitorChannelList {
